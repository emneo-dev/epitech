...
static int is_from_section(const sym_section_64_t *sym_section,
...
static char get_type_sym_bis(const sym_section_64_t *sym_section,
    const Elf64_Shdr *sections,
    const Elf64_Sym *sym)
{
...
static char get_type_sym(const sym_section_64_t *sym_section,
    const Elf64_Shdr *sections,
    size_t index)
{
    char c = '?';
    const Elf64_Sym *sym = &sym_section->symtab[index];
    ...
    if (ELF64_ST_BIND(sym->st_info) == STB_LOCAL && c != '?' && c != 'd'
        && c != 'N')
        c = tolower(c);
    return c;
}
    ...
    size_t index)
{
    ...
    if (sym_section->symtab[index].st_info != STT_FILE &&
        ...
        type = get_type_sym(sym_section, sections, index);
