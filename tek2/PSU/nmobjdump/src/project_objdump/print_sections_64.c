...
static int is_section_to_print(const Elf64_Shdr *section)
{
...
static void print_str(const Elf64_Shdr *section,
...
static void print_hexa(const Elf64_Shdr *section,
    const unsigned char *str,
    size_t i)
{
    for (size_t j = 0; j < 16; j++) {
    ...
    const char *call)
{
    ...
    print_start_64(header, sections, filename);
